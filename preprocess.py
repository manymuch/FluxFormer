...
if __name__ == '__main__':
    rootdir = "/home/zmj/FLUXNET"
    output_dir = "/home/zmj/FluxFormer/data_corrected"
    ...
    for site_path in tqdm(sites):
        ...
        try:
            ...
            print(e)
            continue
        site['Date'] = site['TIMESTAMP_START'].apply(lambda x: pd.to_datetime(str(x), format='%Y%m%d%H%M').date())
        # site['Date'] = site['DateTime'].apply(lambda x: x.date())
        ...
        for date in dates:
            ...
            correction_condition = (site_date["H_F_MDS"]/site_date["LE_F_MDS"])>=0
            rng = site_date['LW_IN_F'][correction_condition] + site_date['SW_IN_F'][correction_condition] - site_date['LW_OUT'][correction_condition] -  site_date['SW_OUT'][correction_condition] - site_date['G_F_MDS'][correction_condition]
            site_date["LE_F_MDS"][correction_condition] = rng/(1+(site_date["H_F_MDS"][correction_condition]/site_date["LE_F_MDS"][correction_condition]))
